...
import com.example.project2.repository.jpql.OrderRepositroy;
...
public class OrderRepositoryTest {
    ...
    public void insertOrderTest() {
        ...
        address.setCity("서울시");
        address.setStreet("152-10");
        address.setZipcode("11017");
        // 2번 member가 3 번 제품을 주문한다.

        LongStream.rangeClosed(1, 3).forEach(i -> {

            Order order = Order.builder().address(address).orderAmount(15).member(JpqlMember.builder().id(2L).build())
    ...
    public void testFindMembers() {
        ...
        // [JpqlMember(id=1, age=1, name=user1), JpqlMember(id=2, age=2, name=user2),
        // ....]
    ...
    public void testAddress() {
        System.out.println(orderRepositroy.findByAddress());

    }

    @Test
    public void testOrders() {
        List<Object[]> result = orderRepositroy.findByOrders();
    ...
    public void queryDslTest() {
        ...
        // Iterable<JpqlMember> members = jpqlMemberRepository.findAll(
        // qMember.name.eq("user3"));

        // user name이 user3인 회원 조회(id 기준으로 내림차순 정렬)
        // booleanBuilder.and(qMember.name.eq("user3"));
        // Iterable<JpqlMember> members = jpqlMemberRepository.findAll(
        // booleanBuilder, Sort.by("id").descending());
